...
**KiCad capture underway — captured as code.** Rather than hand-drawing, the schematic is
generated by [`hardfuzz_v1/gen/`](hardfuzz_v1/gen/) (kigen) and validated with `kicad-cli`
(load + netlist + ERC). The **power sheet** is done and netlist-verified (3 regulators,
rails, feedback dividers). Remaining sheets: USB/FT2232H + config flash, level shifters +
connectors, CAN, and the FPGA (needs an imported FTG256 symbol — see [kicad_parts.md](kicad_parts.md)).
